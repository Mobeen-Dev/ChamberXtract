...
guess = (
    next((c for c in df.columns if "company" in c.lower()), None)
    or next((c for c in df.columns if "name" in c.lower()), None)
    or df.columns[0]
)
column = st.selectbox(
    "Company-name column", df.columns, index=list(df.columns).index(guess)
)
...
with st.sidebar:
    ...
    method = st.radio(
        ...
        help="Agglomerative = no k needed, O(n^2) memory. KMeans = scalable, needs k.",
    )
    analyzer = st.radio(
        "Token unit",
        ["char_wb", "word"],
        index=0,
        help="char_wb tolerates spelling variants; word groups by shared whole words.",
    )
    ...
    if method == "agglomerative":
        ...
        k = st.number_input(
            "Or fix cluster count (0 = use threshold)", 0, 500, 0, step=1
        )
    else:
    ...
    extra_sw = st.text_input(
        "Extra stopwords (comma-separated)", ", ".join(sorted(DEFAULT_STOPWORDS))
    )
...
view = st.radio(
    "Cluster view", ["Grid of charts", "Single cluster detail"], horizontal=True
)

if view == "Grid of charts":
    max_show = st.slider(
        "Clusters to chart (largest first)",
        1,
        min(60, max(1, n_clusters)),
        min(12, max(1, n_clusters)),
    )
    ...
    for i, cid in enumerate(ids):
        ...
        with cols[i % 2]:
            st.altair_chart(
                hbar(d, f"Cluster {cid:02d} - {lab} (n={size})"), width="stretch"
            )
else:
    cid = st.selectbox(
        "Cluster",
        summary.cluster_id.tolist(),
        format_func=lambda c: (
            f"{c:02d} - {summary.loc[summary.cluster_id == c, 'label'].iloc[0]}"
        ),
    )
    st.altair_chart(
        hbar(words[words.cluster_id == cid], f"Cluster {cid:02d}"), width="stretch"
    )
    st.dataframe(
        out_df[out_df.cluster_id == cid].drop(columns=["_normalized"]),
        width="stretch",
        hide_index=True,
    )
...
d3.download_button(
    "Word frequencies",
    words.to_csv(index=False).encode(),
    "cluster_word_counts.csv",
    "text/csv",
)
